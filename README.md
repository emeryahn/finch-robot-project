...
Our project was based on Squid Game's first game from season 1: Red Light, Green Light. The finch acts like the doll and moves its body while flashing either a red or green light. Our objective with this project was to make the finch dance as if it were the doll in the popular Netflix series. Though the project was fun, my partner and I came across many conflicts along the way. Foremost, when trying to connect the robot it wouldn't connect with the code I created. We discovered that we can easily fix this by turning off and on the robot again since it wasn't properly connected. Another problem we came across was a typo leading to an infinite loop error. This was because instead of saying a++ when a = 0, i put i++. That made the section of the code not run. We realized that proof reading the code was vital to running a successful code. After this problem, we no longer came across any issues. In the future, I hope to add actual audio to the robot to say "red light, green light" just like in the movie. This will allow it to be an exact copy of the movie.

[![Squid Game Doll Robot]](https://drive.google.com/file/d/1aibSY8dS2CTpA9phOa4BO3KNBEmWgtos/view?usp=sharing)